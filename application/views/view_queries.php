        ...
        <div class="module-content-section">
	        <div class="table-header-data">
	            ...
	            <div class="left">Showing <?php echo $page_size>sizeof($query)?sizeof($query):$page_size; ?> of <?php echo $total; ?> | Page <?php echo $current_page; ?> of <?php echo $total_pages; ?></div>
	                <div class="right">
					    Records Per Page:  &nbsp; <select name="page_size" class="filter-form-select" style="width:60px;" onchange="window.location.href='<?php echo site_url("student/view_querires/".$id.'?'.$query_string_page_size); ?>&page_size='+$(this).val()">
	                ...
	                <div class="table-container">
						<table>
						  ...
						  <tr>
								...
								<td>
									<a href="<?php echo base_url('student/edit_query/'.$l->id); ?>" class="" title="Edit Listing"><i class="fas fa-pencil-alt" style=" font-size:1.3em; position:relative; top:2px; color:#ffb800;"></i> Edit</a>
									<a href="<?php echo base_url('student/query_delete/'.$l->id.'?cadetno='.$l->cadet_no); ?>" style="color:red;cursor:pointer;margin-left:10px;">Delete</a>
						...
						<div class="data-pagination">
							<?php if($prev_page): ?>
							<a href="<?php echo site_url("student/view_querires/".$id."?page=".$prev_page_num."&".$query_string_pagination); ?>" class="form-button small-button pagination-buttons">Previous</a>
							<?php endif; ?>
							...
							<?php
								...
								if($total_pages>14 and $current_page==$total_pages){ $start=$current_page-10;  $total=($total+$start)>$total_pages?$total_pages:($total+$start); }
									...
									for($i=$start; $i<=$total; $i++){
										...
										if(($i)!=$current_page){
									?>
										<a href="<?php echo site_url("student/view_querires/".$id."?page=".($i-1)."&".$query_string_pagination); ?>" class="form-button small-button pagination-buttons"><?php echo $i; ?></a>
									<?php
								 } else {
									 echo "&nbsp;".$i."&nbsp;";
									 }
							...
							<?php if($next_page): ?>
							<a href="<?php echo site_url("student/view_querires/".$id."?page=".$next_page_num."&".$query_string_pagination); ?>" class="form-button small-button pagination-buttons">Next</a>
							<?php endif; ?>
						</div>
                    </div>
		        </div>
